...
def render():
    st.header("🧾 Traductor ALVIC x CUBRO")
    st.caption("Zenit 06 · 2 outputs: mecanizadas / sin mecanizar · mínimo 100mm")
    ...
    # Path recomendado (ajústalo si cambiaste estructura)
    ...
    if st.button("Traducir y separar", type="primary"):
        if not os.path.exists(db_path):
            st.error(f"No se encuentra la base en: {db_path}")
            return
